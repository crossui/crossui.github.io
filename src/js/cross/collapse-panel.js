...
+function ($) {
  ...
  Collapse.prototype.show = function () {
    ...
    if (actives && actives.length) {
      activesData = actives.data('collapse.panel');
      if (activesData && activesData.transitioning) return
    }

    var startEvent = $.Event('show.collapse.panel');
    this.$element.trigger(startEvent);
    if (startEvent.isDefaultPrevented()) return;

    if (actives && actives.length) {
      Plugin.call(actives, 'hide');
      activesData || actives.data('collapse.panel', null)
    }
  ...
  Collapse.prototype.hide = function () {
    ...
    var startEvent = $.Event('hide.collapse.panel');
    this.$element.trigger(startEvent);
    if (startEvent.isDefaultPrevented()) return;
  ...
  function Plugin(option) {
    return this.each(function () {
      var $this   = $(this);
      var data    = $this.data('collapse.panel');
      var options = $.extend({}, Collapse.DEFAULTS, $this.data(), typeof option == 'object' && option);

      if (!data && options.toggle && option == 'show') options.toggle = false;
      if (!data) $this.data('collapse.panel', (data = new Collapse(this, options)));
  ...
  $(document).on('click', '[data-toggle="collapse"]', function (e) {
    ...
    var data    = $target.data('collapse.panel');
